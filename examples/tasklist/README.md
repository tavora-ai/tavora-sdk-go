...
workspace the API key is scoped to. After that, every agent run in the
workspace auto-discovers these tools (see `internal/agent/mcp.go` in the
...
| `TAVORA_API_KEY` | — | Workspace-scoped API key (`tvr_...`) |
...
- **Workspace-wide tool visibility.** Because Tavora loads MCP servers at
  the workspace level (not per-agent), every agent in this workspace will
...
- **Single-tenant store.** The example's SQLite DB is not per-Tavora-
  workspace. One example process = one logical task-list namespace.
