...
calls, MCP skills, fetch, search — whatever the app has
...
export TAVORA_API_KEY=tvr_...                    # app-scoped API key
```

Create the API key in the Tavora admin UI: sign in at
`/platform`, open your app, go to **Settings → API keys**,
**New key**. Keys start with `tvr_`.

## Run

```bash
# Simplest — inherits app defaults
go run .
...
the core sandbox. MCP servers registered in your app auto-expose
...
- `client.GetApp(ctx)` — sanity check the API key.
...
- API keys grant full access to the scoped app. Treat them like
