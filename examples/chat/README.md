...
calls, MCP skills, fetch, search — whatever the project has
...
export TAVORA_API_KEY=tvr_...                    # project-scoped API key
```

Create the API key in the Tavora admin UI: sign in at
`/platform`, open your project, go to **Settings → API keys**,
**New key**. Keys start with `tvr_`.

## Run

```bash
# Simplest — inherits project defaults
go run .
...
the core sandbox. MCP servers registered in your project auto-expose
...
- `client.GetProject(ctx)` — sanity check the API key.
...
- API keys grant full access to the scoped project. Treat them like
